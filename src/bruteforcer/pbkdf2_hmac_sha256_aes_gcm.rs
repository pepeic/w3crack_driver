...
pub struct PBKDF2_AES_GCM_Request
{
	pub data: Vec<u8>,
	pub iv: Vec<u8>,
	pub salt: Vec<u8>,
...
pub trait PBKDF2_AES_GCM
{
	fn PK2AG_BruteforceCPU(req: &PBKDF2_AES_GCM_Request, passwords: &Vec<String>) -> Result<Option<(String, String)>, String>;
	fn PK2AG_BruteforceGPU(devices: Arc<Vec<GPUDeviceInfo>>, gpupg: Arc<OpenCLProgram>, req: &PBKDF2_AES_GCM_Request, passwords: &Vec<String>) -> Result<Option<(String, String)>, String>;
	fn PK2AG_LoadKernels(devices: &Arc<Vec<GPUDeviceInfo>>) -> Result<OpenCLProgram, String>;
}

impl PBKDF2_AES_GCM for BruteForcer
{
	fn PK2AG_BruteforceCPU(req: &PBKDF2_AES_GCM_Request, passwords: &Vec<String>) -> Result<Option<(String, String)>, String>
	{
		//to perform CPU bruteforce we use fastpbkdf2 crate and aes-gcm crate
		use aes_gcm::{ aead::KeyInit, AesGcm, Nonce, Key, Tag, aes::Aes256 };
		use aead::AeadMutInPlace;

		let mut req_data = req.data.clone();
		let nonce = Nonce::<aead::consts::U16>::from_slice(&req.iv);
		let tag_data = req_data.split_off(req_data.len() - 16);
		let tag = Tag::<aead::consts::U16>::from_slice(&tag_data);
		...
		for possible_pass in passwords
		{
			...
			//try to decrypt now
			let mut decryptor = AesGcm::<Aes256, aead::consts::U16>::new(Key::<AesGcm::<Aes256, aead::consts::U16>>::from_slice(&aes_key));
	...
	fn PK2AG_BruteforceGPU(devices: Arc<Vec<GPUDeviceInfo>>, gpupg: Arc<OpenCLProgram>, req: &PBKDF2_AES_GCM_Request, passwords: &Vec<String>) -> Result<Option<(String, String)>, String>
	{
		...
		let aes_gcm_compute_ghash_krnl = WrappedKernel::clone(gpupg.krnls.get("aes_gcm_compute_ghash").unwrap());
		...
		let total_aes_static_data_size = std::mem::size_of::<AESGCM_TAG>() + std::mem::size_of::<AESGCM_IV>() + std::mem::size_of::<AESGCM_AEAD>() + std::mem::size_of::<cl_uint>() + req.data.len();
		...
		let total_aes_passes_clamped = vec![total_aes_passes_per_iter_with_static_data, total_passes_per_iter, aes_gcm_compute_ghash_krnl.mgws].into_iter().min().unwrap_or(0);
		...
		let aes_crypt_data;
		{
			...
			let mut iv_buffer = AESGCM_IV
			{
				iv: unsafe { std::mem::MaybeUninit::uninit().assume_init() },
				iv_len: req_iv_len as u8
			};
			...
			//init some data which was skipped before
			iv_buffer.iv[..req_iv_len].copy_from_slice(&req.iv);
			...
			unsafe 
			{
				...
				aes_iv = Buffer::<AESGCM_IV>::create(&gpupg.ctx, flags, 1, std::ptr::addr_of_mut!(iv_buffer).cast::<c_void>()).map_err(|e| e.to_string())?;
				...
				aes_crypt_data = Buffer::<u8>::create(&gpupg.ctx, flags, req_data.len(), req_data.as_mut_ptr().cast::<c_void>()).map_err(|e| e.to_string())?;
			}
		}
		...
		for _ in 0..total_iters_with_salt
		{
			...
			for aes_iter in 0..total_aes_iters_per_pbkdf2_iter
			{
				...
				let event = unsafe
				{
					let mut kernel = ExecuteKernel::new(&aes_gcm_compute_ghash_krnl.krnl);
					// let guard = aes_gcm_compute_ghash_krnl.mtx.lock();
					kernel.set_arg_svm(aes_key.as_ptr())
						.set_arg(&aes_iv.get())
						.set_arg(&aes_crypt_data.get())
						.set_arg(&(req_data.len() as cl_uint))
				...
				for i in 0..passes_this_time
				{
					if results[i] == 0
					{
						...
						//try to decrypt now
						let mut decryptor = AesGcm::<Aes256, aead::consts::U16>::new(Key::<AesGcm::<Aes256, aead::consts::U16>>::from_slice(&key));
						let mut data_copy = req_data.clone();
						if let Ok(_) = decryptor.decrypt_in_place_detached(nonce, b"", &mut data_copy, tag)
						{
							//we found it! Signal that we found the needed password
	...
	fn PK2AG_LoadKernels(devices: &Arc<Vec<GPUDeviceInfo>>) -> Result<OpenCLProgram, String>
	{
		...
		let aes_gcm_single = String::from_utf8(include_bytes!("./cl/aes_gcm_single.cl").to_vec()).map_err(|e| e.to_string())?;
		...
		let program2 = Program::create_and_build_from_source(&context, &aes_gcm_single, CL_STD_2_0).map_err(|e| e.to_string())?;
		let aes_gcm_compute_ghash_krnl = Kernel::create(&program2, "aes_gcm_compute_ghash").map_err(|e| e.to_string())?;
		...
		let aes_gcm_compute_ghash_mgws = devices.iter().map(|d| aes_gcm_compute_ghash_krnl.get_work_group_size(d.device.id()).unwrap_or(1)).collect::<Vec<_>>().into_iter().min().unwrap_or(1);
		...
		kernels.insert("aes_gcm_compute_ghash", WrappedKernel{ krnl: aes_gcm_compute_ghash_krnl, mgws: aes_gcm_compute_ghash_mgws, mtx: Mutex::new(()) });
