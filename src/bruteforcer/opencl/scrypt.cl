...
static inline void salsa(__private const uint4 Bx[4], __private uint4 B[4]);
static inline void BlockMix(__private T_Block* B);
...
__kernel void ROMix(int N, __global T_Block* Xs, __global T_Block* Vs, __global T_Block* outputs)
{
	__private unsigned int id = get_global_id(0);
	__private T_Block X = Xs[id];
	__private int i, j, k, v_idx;

	//offset the index by the N, this is required for parallel computations
	__private int v_idx_offset = id * N;

	for (i = 0, v_idx = v_idx_offset; i < N; ++i, ++v_idx)
	{
		Vs[v_idx] = X;
		BlockMix(&X);
	}
