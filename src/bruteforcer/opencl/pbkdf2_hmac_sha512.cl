...
#include "misc.hcl"
#include "sha512.hcl"
#include "pbkdf2_hmac_sha512.hcl"

inline void _phs512_preproc(__global const uchar *key, uint keylen, ulong *state, ulong mask)
{
	uint i;
	ulong W[16];
...
inline void _phs512_hmac(ulong *output, ulong *ipad_state, ulong *opad_state, __global const uchar *salt, uint saltlen)
{
	uint i;
	ulong W[16];
	ulong A, B, C, D, E, F, G, H, t;
...
__kernel void pbkdf2_sha512_loop(__global state_t *state, __global crack_t *out)
{
	...
	else
	{
		...
		#pragma unroll
		for (i = 0; i < 8; i++)
			out[idx].hash[i] = SWAP64(tmp_out[i]);
	}
}

__kernel void pbkdf2_sha512_kernel(__global const pass_t *inbuffer, __global const salt_t *gsalt, __global state_t *state)
{
	ulong ipad_state[8];
	ulong opad_state[8];
	ulong tmp_out[8];
