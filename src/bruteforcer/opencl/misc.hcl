...
#define PUTCHAR_BE64(buf, index, val) (buf)[(index)>>3] = ((buf)[(index)>>3] & ~(0xffU << ((((index) & 7) ^ 7) << 3))) + ((val) << ((((index) & 7) ^ 7) << 3))
...
#define XORCHAR_64(buf, index, val) (buf)[(index)>>3] = ((buf)[(index)>>3]) ^ (((ulong)(val)) << (((index) & 7) << 3))
#define XORCHAR_64BE(buf, index, val) (buf)[(index)>>3] = ((buf)[(index)>>3]) ^ (((ulong)(val)) << ((((index) & 7) ^ 7) << 3))
...
#define PUTCHAR_BE64(buf, index, val) ((uchar*)(buf))[(index) ^ 7] = (val)
...
#define XORCHAR_64(buf, index, val) ((uchar*)(buf))[(index)] ^= ((ulong)(val))
#define XORCHAR_64BE(buf, index, val) ((uchar*)(buf))[(index) ^ 7] ^= ((ulong)(val))
...
#define memset_macro(dst, value, count) do {	  \
		uint _memset_c = count; \
		for (uint _memset_i = 0; _memset_i < _memset_c; _memset_i++) \
			(dst)[_memset_i] = value; \
	} while (0)
